...
export default function PricingTable({ block }: { block: PageBlock }) {
  ...
  const plans = (props.plans || []).map((plan) => ({
    ...
    problemSolved: getLocalizedString(plan.problemSolved, locale),
    subText: plan.subText ? getLocalizedString(plan.subText, locale) : "",
    foundation: (plan.foundation || []).map((f) => getLocalizedString(f, locale)),
    monthly: (plan.monthly || []).map((m) => getLocalizedString(m, locale)),
    idealIf: getLocalizedString(plan.idealIf, locale),
    ctaText: getLocalizedString(plan.ctaText, locale),
        ...
        {plans.map((plan, index) => (
          ...
          >
            ...
            <div className="mt-6">
              ...
              <p className="mt-3 text-[14px] leading-7 text-[#4b5563] font-normal">
                {plan.problemSolved}
              </p>
            </div>
            ...
            <div className="mt-6 flex-1 flex flex-col">
              ...
              {plan.subText && (
                <p className="text-[12px] text-[#6b7280] mb-4 font-normal">
                  {plan.subText}
                </p>
              )}

              <div className="space-y-4 flex-1">
                {plan.foundation.length > 0 && (
                  <div>
            ...
            <div className="mt-6 border-t border-[#d1d5db] pt-4">
              ...
              <p className="mt-2 text-[14px] text-[#4b5563] font-normal leading-relaxed">
                {plan.idealIf}
              </p>
              <a href={plan.ctaHref}>
